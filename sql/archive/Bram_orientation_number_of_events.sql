------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------
-- Number of Events per type, I don’t know if for your analysis you want 
-- the total amount then you can remove the case when statement
------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------
-- Total events per student
WITH EventCategorieCTE AS (
select distinct
st.STUDENTNUMMER,
...
SELECT
    STUDENTNUMMER,
    EventCategorie,
    COUNT(DISTINCT Event_start_datum) AS AantalUniekeEventData
FROM EventCategorieCTE
GROUP BY STUDENTNUMMER, EventCategorie
